...
def sync_paytraq_to_pipedrive():
    ...
    if not data or not data.get('order_name', '').startswith('test'):
        return jsonify({"message": "Not a test order, skipping"}), 200

    email = data.get('email', 'test@example.com')
    person = search_person_by_email(email)

    if not person:
        person = create_person(data.get('client_name', 'Test Client'), email)

    deal = create_deal(data.get('order_name', 'Test Deal'), person['id'])

    product = add_product_to_pipedrive('Test Product', 2.00)
    attach_product_to_deal(deal['id'], product['id'], 2.00)

    return jsonify({"message": "Deal created successfully!"}), 200
